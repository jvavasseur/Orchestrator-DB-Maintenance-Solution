...
BEGIN
    BEGIN TRY 
        ...
        BEGIN
            ...
            END TRY
            BEGIN CATCH
                SET @message = N'ERROR[TS0]: error(s) occured while checking ArchivingListOrchestratorDBTables table';
                THROW;
            END CATCH
        ...
        BEGIN
            BEGIN TRY
                EXEC [Maintenance].[SetSourceTable]
                    @SynonymName = N'Synonym_ArchivingDatasourceForOrchestratorDB', @SynonymSchema = N'Maintenance'
                    , @SourceTableFullParts = N'Maintenance.ArchivingListOrchestratorDBTables'
            ...
            END TRY
            BEGIN CATCH
                SET @message = N'ERROR[TS0]: error(s) occured while checking ArchivingListOrchestratorDBTables Synonym';
                THROW;
            END CATCH
        END
        INSERT INTO @json_errors([procedure], [severity], [message]) SELECT N'[Maintenance].[CreateArchivingExternalTable]', [severity], [message] FROM OPENJSON(@outputMessages) WITH([Message] nvarchar(MAX), [Severity] int)
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE CursorTables CURSOR FAST_FORWARD LOCAL FOR SELECT [group], [schema], [table], [exists], [isvalid], [columns] FROM [Maintenance].[Synonym_ArchivingDatasourceForOrchestratorDB];
                OPEN CursorTables;
            ...
            END TRY
            BEGIN CATCH
                SET @message = N'ERROR[XT0]: error(s) occured while processing tables from ArchivingListOrchestratorDBTables';
                THROW;
            END CATCH
        END
    END TRY
...
EXEC [Maintenance].[SetArchiveDBSourceTables] @DataSource = 'ArchivingDatasourceForOrchestratorDB'
GO
/*
select * from maintenance.Synonym_ArchivingDatasourceForOrchestratorDB
