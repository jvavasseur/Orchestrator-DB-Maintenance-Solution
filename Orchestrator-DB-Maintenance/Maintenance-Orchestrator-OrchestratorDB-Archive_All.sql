...
-- ### [Hash]: 474f556 [SHA256-6EE98B3042A9910640D083E6C64C8497DD91C3EEEF95D376244C4649AFB88E1C]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
BEGIN
    ...
    ( 
        ...
        FROM (
            ...
            UNION ALL SELECT (SELECT (SELECT MAX(Id) + 2 FROM @json_errors)), @message, 16, 1 WHERE @message IS NOT NULL
        ) jsn
        FOR JSON PATH)
    --    , N'[]')
    ;
...
-- ### [Hash]: 3311d03 [SHA256-6D7AF63ACC27CA5E74293E98BEA430B3518AC8245ABB0905B3A8A1C83A8FF9A2]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
-- ### [Hash]: 3311d03 [SHA256-DCA14692C6CC1157F856212D1FD61DFD6BFB3A4B12EF03CEAA7A8D0332707B14]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
/*            -- Check Dry Run
    ...
    BEGIN TRY 
        ...
        BEGIN TRY
            ...
            FROM [Maintenance].[Synonym_Archive_Sync_Logs] 
            ...
            INNER JOIN [Maintenance].[Synonym_Archive_Sync_Logs] syn ON syn.Id = sts.SyncId
        ...
        BEGIN 
            ...
            BEGIN;
                ...
                BEGIN
                    ...
                    SELECT TOP(@maxLoopDeleteRows) Id FROM [Maintenance].[Synonym_Archive_Delete_Logs] WHERE [SyncId] = @cursorSyncId AND Id >= @firstId ORDER BY Id ASC;
                    IF @@ROWCOUNT = 0
                    BEGIN
...
-- ### [Source]: _src/Archive/OrchestratorDB/Jobs/Table_OrchestratorDB.Maintenance.ASyncStatus_Jobs.sql
-- ### [Hash]: 0859ec6 [SHA256-FFEDCB9433D07C5DE2FF16F52738295169986267EB104A4E5303A6CC74ADD67E]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsJobs]
----------------------------------------------------------------------------------------------------
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ValidateASyncArchiveObjectsJobs]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsJobs] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsJobs]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsJobs] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsJobs]'
GO
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ASyncCleanupJobs]
----------------------------------------------------------------------------------------------------

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ASyncCleanupJobs]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ASyncCleanupJobs] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ASyncCleanupJobs]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ASyncCleanupJobs] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ASyncCleanupJobs]'
GO  
...
/*            -- Check Dry Run
        ...
        BEGIN
            -- Check Synonyms and source/Archive tables
            BEGIN TRY            
                EXEC [Maintenance].[ValidateASyncArchiveObjectsJobs] @Messages = @synonymMessages OUTPUT, @IsValid = @synonymIsValid OUTPUT;
    ...
    BEGIN TRY 
        ...
        BEGIN 
            ...
            BEGIN;
                SET @message = N'Sync Id [' + CAST(@cursorSyncId AS nvarchar(100)) + N']: ';
                IF EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_Jobs] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1)
                BEGIN
                    SELECT @message = @message + IIF(@cursorIsSynced = 1, N' remove (already deleted and synced)', N'skip (deleted but not synced yet)')
                    EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;

                    DELETE FROM [Maintenance].[ASyncStatus_Jobs] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1 AND @cursorIsSynced = 1;
                    FETCH CursorSyncIds INTO @cursorSyncId, @cursorDeleteAfterDatetime, @cursorFirstASyncId, @cursorLastAsyncId, @cursorCountASyncIds, @cursorIsDeleted, @cursorIsSynced;
                    CONTINUE;
                END
                ...
                SELECT @message = @message + N'Start';
                EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;
                INSERT INTO [Maintenance].[ASyncStatus_Jobs]([SyncId], [FirstASyncId]) SELECT @cursorSyncId, @cursorFirstASyncId WHERE NOT EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_Jobs] WHERE [SyncId] = @cursorSyncId)
                SELECT @firstId = COALESCE([LastASyncId] + 1, [FirstASyncId], @cursorFirstASyncId) FROM [Maintenance].[ASyncStatus_Jobs] WHERE [SyncId] = @cursorSyncId
                UPDATE [Maintenance].[ASyncStatus_Jobs] SET [FirstASyncId] = @firstId WHERE [SyncId] = @cursorSyncId
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        UPDATE stt SET [IsDeleted] = 1, [DeletedOnDate] = SYSDATETIME() FROM [Maintenance].[ASyncStatus_Jobs] stt WHERE [SyncId] = @cursorSyncId;
                        BREAK;
                    END
                    SELECT @firstId = MIN(Id), @lastId = MAX(Id), @countIds = COUNT(*) FROM #tempIds;

                    BEGIN TRAN
                    DELETE lgs FROM [dbo].[Jobs] lgs INNER JOIN #tempIds ids ON ids.Id = lgs.Id;                    
                    UPDATE stt SET [LastAsyncId] = @lastId FROM [Maintenance].[ASyncStatus_Jobs] stt WHERE [SyncId] = @cursorSyncId;
                    IF @@TRANCOUNT > 0 COMMIT
                    TRUNCATE TABLE #tempIds;
...
-- ### [Source]: _src/Archive/OrchestratorDB/Queues/Table_OrchestratorDB.Maintenance.ASyncStatus_Queues.sql
-- ### [Hash]: dc39c27 [SHA256-98396B94C9BF055916A041D6C3256F16713E834717932C192A3C30C796A76303]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsQueues]
----------------------------------------------------------------------------------------------------
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ValidateASyncArchiveObjectsQueues]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsQueues] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsQueues]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsQueues] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsQueues]'
GO
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ASyncCleanupQueues]
----------------------------------------------------------------------------------------------------

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ASyncCleanupQueues]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ASyncCleanupQueues] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ASyncCleanupQueues]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ASyncCleanupQueues] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ASyncCleanupQueues]'
GO  
...
/*            -- Check Dry Run
        ...
        BEGIN
            -- Check Synonyms and source/Archive tables
            BEGIN TRY            
                EXEC [Maintenance].[ValidateASyncArchiveObjectsQueues] @Messages = @synonymMessages OUTPUT, @IsValid = @synonymIsValid OUTPUT;
    ...
    BEGIN TRY 
        ...
        BEGIN 
            ...
            BEGIN;
                SET @message = N'Sync Id [' + CAST(@cursorSyncId AS nvarchar(100)) + N']: ';
                IF EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_Queues] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1)
                BEGIN
                    SELECT @message = @message + IIF(@cursorIsSynced = 1, N' remove (already deleted and synced)', N'skip (deleted but not synced yet)')
                    EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;

                    DELETE FROM [Maintenance].[ASyncStatus_Queues] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1 AND @cursorIsSynced = 1;
                    FETCH CursorSyncIds INTO @cursorSyncId, @cursorDeleteAfterDatetime, @cursorFirstASyncId, @cursorLastAsyncId, @cursorCountASyncIds, @cursorIsDeleted, @cursorIsSynced;
                    CONTINUE;
                END
                ...
                SELECT @message = @message + N'Start';
                EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;
                INSERT INTO [Maintenance].[ASyncStatus_Queues]([SyncId], [FirstASyncId]) SELECT @cursorSyncId, @cursorFirstASyncId WHERE NOT EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_Queues] WHERE [SyncId] = @cursorSyncId)
                SELECT @firstId = COALESCE([LastASyncId] + 1, [FirstASyncId], @cursorFirstASyncId) FROM [Maintenance].[ASyncStatus_Queues] WHERE [SyncId] = @cursorSyncId
                UPDATE [Maintenance].[ASyncStatus_Queues] SET [FirstASyncId] = @firstId WHERE [SyncId] = @cursorSyncId
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        UPDATE stt SET [IsDeleted] = 1, [DeletedOnDate] = SYSDATETIME() FROM [Maintenance].[ASyncStatus_Queues] stt WHERE [SyncId] = @cursorSyncId;
                        BREAK;
                    END
                    SELECT @firstId = MIN(Id), @lastId = MAX(Id), @countIds = COUNT(*) FROM #tempIds;

                    BEGIN TRAN
                    DELETE ent FROM [dbo].[QueueItemComments] src INNER JOIN #tempIds ids ON ids.Id = src.QueueItemId;                    
                    DELETE ent FROM [dbo].[QueueItemEvents] src INNER JOIN #tempIds ids ON ids.Id = src.QueueItemId;                    
                    DELETE lgs FROM [dbo].[QueueItems] src INNER JOIN #tempIds ids ON ids.Id = src.Id;                    
                    UPDATE stt SET [LastAsyncId] = @lastId FROM [Maintenance].[ASyncStatus_Queues] stt WHERE [SyncId] = @cursorSyncId;
                    IF @@TRANCOUNT > 0 COMMIT
                    TRUNCATE TABLE #tempIds;
...
----------------------------------------------------------------------------------------------------
-- ### [Object]: TABLE [Maintenance].[ASyncStatus_AuditLogs]
-- ### [Version]: 2023-09-07T18:14:12+02:00
-- ### [Source]: _src/Archive/OrchestratorDB/AuditLogs/Table_OrchestratorDB.Maintenance.ASyncStatus_AuditLogs.sql
-- ### [Hash]: 914e4af [SHA256-DDA36C336D9E3B57327056C2532B9DA4DC44AAD022F6E61FADAA22ED58552A71]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
----------------------------------------------------------------------------------------------------
IF NOT EXISTS(SELECT 1 FROM sys.tables WHERE name = N'ASyncStatus_AuditLogs' AND SCHEMA_NAME(schema_id) = N'Maintenance')
BEGIN
    PRINT '  + CREATE TABLE: [Maintenance].[ASyncStatus_AuditLogs]';

	CREATE TABLE [Maintenance].[ASyncStatus_AuditLogs](
		[SyncId] [bigint] NOT NULL
		, [IsDeleted] [bit] NOT NULL CONSTRAINT [DF_Maintenance.ASyncStatus_AuditLogs.IsDeleted] DEFAULT 0
		, [DeletedOnDate] [datetime] NULL
		, [FirstASyncId] [bigint] NULL
		, [LastAsyncId] [bigint] NULL
		, CONSTRAINT [PK_Maintenance.ASyncStatus_AuditLogs] PRIMARY KEY CLUSTERED ([SyncId] ASC)
			WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
	) ON [PRIMARY]
END
ELSE PRINT '  = Table already exists: [Maintenance].[ASyncStatus_AuditLogs]';
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsAuditLogs]
----------------------------------------------------------------------------------------------------
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ValidateASyncArchiveObjectsAuditLogs]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsAuditLogs] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsAuditLogs]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsAuditLogs] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ValidateASyncArchiveObjectsAuditLogs]'
GO

ALTER PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsAuditLogs]
----------------------------------------------------------------------------------------------------
-- ### [Object]: PROCEDURE [Maintenance].[ValidateASyncArchiveObjectsAuditLogs]
-- ### [Version]: 2023-10-18T14:34:46+02:00
-- ### [Source]: _src/Archive/OrchestratorDB/AuditLogs/Procedure_OrchestratorDB.Maintenance.ValidateASyncArchiveObjectsAuditLogs.sql
-- ### [Hash]: 3311d03 [SHA256-93695A543077B7B6682A3B9D5942340A393037920D92E61A5D5686DEAB61E017]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
----------------------------------------------------------------------------------------------------
-- DROP PROCEDURE [Maintenance].[ASyncCleanupAuditLogs]
----------------------------------------------------------------------------------------------------

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[ASyncCleanupAuditLogs]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[ASyncCleanupAuditLogs] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[ASyncCleanupAuditLogs]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[ASyncCleanupAuditLogs] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[ASyncCleanupAuditLogs]'
GO  
...
-- ### [Hash]: 3311d03 [SHA256-8A47F199EB768ACCE469D89A9C93B1545A7B1DFF2457F85C31045DE5E209A26A]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
/*            -- Check Dry Run
        ...
        BEGIN
            -- Check Synonyms and source/Archive tables
            BEGIN TRY            
                EXEC [Maintenance].[ValidateASyncArchiveObjectsAuditLogs] @Messages = @synonymMessages OUTPUT, @IsValid = @synonymIsValid OUTPUT;
    ...
    BEGIN TRY 
        ...
        BEGIN TRY
            INSERT INTO @tempListSync([Id], [DeleteAfterDatetime], [FirstASyncId], [LastAsyncId], [CountASyncIds], [IsDeleted], [IsSynced])
            SELECT Id, DeleteAfterDatetime, FirstASyncId, LastASyncId, CountASyncIds, IsDeleted, IsSynced
            FROM [Maintenance].[Synonym_Archive_Sync_AuditLogs] 
            WHERE ( CountASyncIds > 0 AND IsArchived = 1 AND IsDeleted <> 1 AND DeleteAfterDatetime < @startTime )
            UNION
            SELECT syn.Id, syn.DeleteAfterDatetime, syn.FirstASyncId, syn.LastASyncId, syn.CountASyncIds, syn.IsDeleted, syn.IsSynced
            FROM [Maintenance].[ASyncStatus_AuditLogs] sts 
            INNER JOIN [Maintenance].[Synonym_Archive_Sync_AuditLogs] syn ON syn.Id = sts.SyncId
           WHERE sts.IsDeleted = 1;
        ...
        BEGIN 
            ...
            BEGIN;
                SET @message = N'Sync Id [' + CAST(@cursorSyncId AS nvarchar(100)) + N']: ';
                IF EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_AuditLogs] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1)
                BEGIN
                    SELECT @message = @message + IIF(@cursorIsSynced = 1, N' remove (already deleted and synced)', N'skip (deleted but not synced yet)')
                    EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;

                    DELETE FROM [Maintenance].[ASyncStatus_AuditLogs] WHERE [SyncId] = @cursorSyncId AND [IsDeleted] = 1 AND @cursorIsSynced = 1;
                    FETCH CursorSyncIds INTO @cursorSyncId, @cursorDeleteAfterDatetime, @cursorFirstASyncId, @cursorLastAsyncId, @cursorCountASyncIds, @cursorIsDeleted, @cursorIsSynced;
                    CONTINUE;
                END
                ...
                SELECT @message = @message + N'Start';
                EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;
                INSERT INTO [Maintenance].[ASyncStatus_AuditLogs]([SyncId], [FirstASyncId]) SELECT @cursorSyncId, @cursorFirstASyncId WHERE NOT EXISTS(SELECT 1 FROM [Maintenance].[ASyncStatus_AuditLogs] WHERE [SyncId] = @cursorSyncId)
                SELECT @firstId = COALESCE([LastASyncId] + 1, [FirstASyncId], @cursorFirstASyncId) FROM [Maintenance].[ASyncStatus_AuditLogs] WHERE [SyncId] = @cursorSyncId
                UPDATE [Maintenance].[ASyncStatus_AuditLogs] SET [FirstASyncId] = @firstId WHERE [SyncId] = @cursorSyncId

                SET @totalIds = 0;
                WHILE 0 >= 0
                BEGIN
                    INSERT INTO #tempIds(Id)
                    SELECT TOP(@maxLoopDeleteRows) Id FROM [Maintenance].[Synonym_Archive_Delete_AuditLogs] WHERE [SyncId] = @cursorSyncId AND Id >= @firstId ORDER BY Id ASC;
                    IF @@ROWCOUNT = 0
                    BEGIN
                        SET @message = SPACE(@tab * 1) + 'Cleanup finished (total = ' + CAST(@totalIds AS nvarchar(100)) + N')';
                        EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;

                        UPDATE stt SET [IsDeleted] = 1, [DeletedOnDate] = SYSDATETIME() FROM [Maintenance].[ASyncStatus_AuditLogs] stt WHERE [SyncId] = @cursorSyncId;
                        BREAK;
                    END
                    SELECT @firstId = MIN(Id), @lastId = MAX(Id), @countIds = COUNT(*) FROM #tempIds;

                    BEGIN TRAN
                    DELETE ent FROM [dbo].[AuditLogsEntities] ent INNER JOIN #tempIds ids ON ids.Id = ent.AuditLogId;                    
                    DELETE lgs FROM [dbo].[AuditLogs] lgs INNER JOIN #tempIds ids ON ids.Id = lgs.Id;                    
                    UPDATE stt SET [LastAsyncId] = @lastId FROM [Maintenance].[ASyncStatus_AuditLogs] stt WHERE [SyncId] = @cursorSyncId;
                    IF @@TRANCOUNT > 0 COMMIT
                    TRUNCATE TABLE #tempIds;
...
-- ### [Hash]: 3311d03 [SHA256-8F05EAAE62F408C3CD465B79F874A5A45756F98F2D45EA2DEF1D761D9E28228F]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
BEGIN
    BEGIN TRY 
        ...
        -- Settings
        ----------------------------------------------------------------------------------------------------
        DECLARE @synonymASyncDeleteName nvarchar(256) = N'Synonym_Archive_Delete_RobotLicenseLogs';
        DECLARE @synonymASyncDeleteSchema nvarchar(256) = N'Maintenance';
        DECLARE @synonymArchiveSyncName nvarchar(256) = N'Synonym_Archive_Sync_RobotLicenseLogs';
        DECLARE @synonymArchiveSyncSchema nvarchar(256) = N'Maintenance';
...
-- ### [Hash]: 3311d03 [SHA256-E6A1FF5F42861F41BF0546FA70E8C83AE728D3B6704F1A1F9854D4C22E405785]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
/*            -- Check Dry Run
    ...
    BEGIN TRY 
        ...
        BEGIN TRY
            INSERT INTO @tempListSync([Id], [DeleteAfterDatetime], [FirstASyncId], [LastAsyncId], [CountASyncIds], [IsDeleted], [IsSynced])
            SELECT Id, DeleteAfterDatetime, FirstASyncId, LastASyncId, CountASyncIds, IsDeleted, IsSynced
            FROM [Maintenance].[Synonym_Archive_Sync_RobotLicenseLogs] 
            WHERE ( CountASyncIds > 0 AND IsArchived = 1 AND IsDeleted <> 1 AND DeleteAfterDatetime < @startTime )
            UNION
            SELECT syn.Id, syn.DeleteAfterDatetime, syn.FirstASyncId, syn.LastASyncId, syn.CountASyncIds, syn.IsDeleted, syn.IsSynced
            FROM [Maintenance].[ASyncStatus_RobotLicenseLogs] sts 
            INNER JOIN [Maintenance].[Synonym_Archive_Sync_RobotLicenseLogs] syn ON syn.Id = sts.SyncId
           WHERE sts.IsDeleted = 1;
        ...
        BEGIN 
            ...
            BEGIN;
                ...
                BEGIN
                    INSERT INTO #tempIds(Id)
                    SELECT TOP(@maxLoopDeleteRows) Id FROM [Maintenance].[Synonym_Archive_Delete_RobotLicenseLogs] WHERE [SyncId] = @cursorSyncId AND Id >= @firstId ORDER BY Id ASC;
                    IF @@ROWCOUNT = 0
                    BEGIN
                        SET @message = SPACE(@tab * 1) + 'Cleanup finished (total = ' + CAST(@totalIds AS nvarchar(100)) + N')';
                        EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = @message, @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = @logToTable, @MessagesStack = @MessagesStack OUTPUT;
