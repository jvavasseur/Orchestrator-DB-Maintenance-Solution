...
----------------------------------------------------------------------------------------------------
-- PROCEDURE [Maintenance].[CleanupLedger]
----------------------------------------------------------------------------------------------------

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Maintenance].[CleanupLedger]') AND type in (N'P'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [Maintenance].[CleanupLedger] AS'
    PRINT '  + CREATE PROCEDURE: [Maintenance].[CleanupLedger]';
END
ELSE PRINT '  = PROCEDURE [Maintenance].[CleanupLedger] already exists' 
GO

PRINT '  ~ UPDATE PROCEDURE: [Maintenance].[CleanupLedger]'
GO

ALTER PROCEDURE [Maintenance].[CleanupLedger]
----------------------------------------------------------------------------------------------------
-- ### [Object]: PROCEDURE [Maintenance].[CleanupLedger]
-- ### [Version]: 2023-02-09T15:51:30+00:00
-- ### [Source]: _src/Cleanup/Procedure_Maintenance.CleanupLedger.sql
-- ### [Hash]: 5c0e7ad [SHA256-E49E824761CF296D48A9D3395D40D2A31BEF3C7B67EDD1FE24E1D1C45A4ECFB0]
-- ### [Docs]: https://???.???
----------------------------------------------------------------------------------------------------
    ...
    , @CleanupBelowId bigint = NULL -- Provide Max Ledger Id to procedure
...
*/
        ...
        END
		*/
        /*-- Check ALTER Permission on [dbo].[Ledger]
        INSERT INTO @messages ([Message], Severity, [State])
        ...
        LEFT JOIN sys.fn_my_permissions(N'[dbo].[Ledger]', N'OBJECT') eff ON eff.subentity_name = p.subentity_name AND eff.permission_name = p.permission_name
        ...
        BEGIN
            INSERT INTO @messages ([Message], Severity, [State]) VALUES
                ...
                , (N'When set to Y, @DisableIndex or @RebuildIndex require ALTER permission on [dbo].[Ledger] table', 16, 1);
        END*/

        -- Check SELECT & DELETE permission on [dbo].[Ledger]
        INSERT INTO @messages ([Message], Severity, [State])
        SELECT 'Permission not effectively granted: ' + UPPER(p.permission_name), 10, 1
        ...
        BEGIN
            INSERT INTO @messages ([Message], Severity, [State]) VALUES
                (N'Error: missing permission', 10, 1)
                , (N'SELECT and DELETE permissions are required on [dbo].[Ledger] table', 16, 1);
        END
...
*/
            ...
            BEGIN 
                ...
                INSERT INTO @messages ([Message], Severity, [State])
                SELECT 'Use Provided Max LedgerId: ' + COALESCE(CAST(@MaxId AS nvarchar(MAX)), '-'), 10, 1;
            END
            ELSE
            BEGIN
                SELECT @MaxId = MAX(LedgerId) FROM [dbo].[Ledger] WITH (READPAST) WHERE CreationTime < @MaxCreationTime;

                INSERT INTO @messages ([Message], Severity, [State])
                SELECT 'Get Max Ledger Id from Table: ' + COALESCE(CAST(@MaxId AS nvarchar(MAX)), '-'), 10, 1;
            END
        ...
        IF @logToTable = 1 
        BEGIN
            INSERT INTO [Maintenance].[Runs]([Type], [Info], [StartTime]) SELECT N'Cleanup Ledger', N'PROCEDURE ' + @procName, @startTime;
    ...
    BEGIN TRY 
        ...
        BEGIN
            BEGIN TRY
                ...
                -- Get Ids
                BEGIN TRY
					IF @minId >= @curIdEnd SELECT @curIdStart = MIN(id), @curIdEnd = MAX(id) FROM (SELECT TOP(@deleteTopRows) id = LedgerId FROM [dbo].[Ledger] WITH (READPAST) WHERE LedgerId >= @minId AND LedgerId <= @MaxId AND CreationTime < @MaxCreationTime ORDER BY Id ASC) l(id);
                END TRY
                BEGIN CATCH
	                SELECT @ERROR_NUMBER = ERROR_NUMBER(), @ERROR_SEVERITY = ERROR_SEVERITY(), @ERROR_STATE = ERROR_STATE(), @ERROR_PROCEDURE = ERROR_PROCEDURE(), @ERROR_LINE = ERROR_LINE(), @ERROR_MESSAGE = ERROR_MESSAGE();
                    IF @@TRANCOUNT > 0
                ...
                BEGIN TRY
                    ...
                    WITH del AS (
                        SELECT TOP(@deleteTopRows) LedgerId
                        FROM [dbo].[Ledger] WITH (READPAST, INDEX([PK_dbo.Ledger]))
                        WHERE LedgerId >= @curIdStart AND LedgerId <= @MaxId AND CreationTime < @MaxCreationTime 
...
-- ### [Version]: 2023-01-31T12:58:44+00:00
-- ### [Source]: _src/Cleanup/Procedure_Maintenance.CleanupLedgerDeliveries.sql
-- ### [Hash]: d0ca361 [SHA256-90FE10F831BB3A4A5207E4FADD085761C4544DD4F10B971ACC248C57E0D2AD81]
-- ### [Docs]: https://???.???
----------------------------------------------------------------------------------------------------
    @HoursToKeep int = NULL -- i.e. 168h = 7*24h = 7 days => value can't be NULL and must be bigger than 0 if @CleanupBeforeDate is not set
    , @CleanupBeforeDate datetime = NULL -- Use either @CleanupBeforeDate or @HoursToKeep BUT not both
...
-- ### [Hash]: d0ca361 [SHA256-29664BCF7DBC4F05EF3DC8413E42B5538BCC96BB0A3D8CCCD87CF2077BEE451D]
-- ### [Docs]: https://???.???
----------------------------------------------------------------------------------------------------
    @HoursToKeep int = NULL -- i.e. 168h = 7*24h = 7 days => value can't be NULL and must be bigger than 0 if @CleanupBeforeDate is not set
    , @CleanupBeforeDate datetime = NULL -- Use either @CleanupBeforeDate or @HoursToKeep BUT not both
...
-- ### [Hash]: d0ca361 [SHA256-DA1A7F27F4DB8766166507D83C752096C305007C046375948684FFBD7C1C1691]
-- ### [Docs]: https://???.???
----------------------------------------------------------------------------------------------------
    @HoursToKeep int = NULL -- i.e. 168h = 7*24h = 7 days => value can't be NULL and must be bigger than 0 if @CleanupBeforeDate is not set
    , @CleanupBeforeDate datetime = NULL -- Use either @CleanupBeforeDate or @HoursToKeep BUT not both
...
BEGIN
    ...
    BEGIN TRY 
		...
		IF @levelVerbose >= @VerboseBelowLevel 
        BEGIN
            EXEC [Maintenance].[AddRunMessage] @RunId = @runId, @Procedure = @procName, @Message = N'Cleanup in progress... (Verbose not set)', @Severity = 10, @State = 1, @VerboseLevel = @levelVerbose, @LogToTable = 0, @MessagesStack = @MessagesStack OUTPUT;
        END
...
-- ### [Hash]: d0ca361 [SHA256-E7BF0EC646C409056FA64D15B20CA69B84C0E5A5F3F22B2EEEE0C30B4756B51E]
-- ### [Docs]: https://???.???
----------------------------------------------------------------------------------------------------
    @HoursToKeep int = NULL -- i.e. 168h = 7*24h = 7 days => value can't be NULL and must be bigger than 0 if @CleanupBeforeDate is not set
    , @CleanupBeforeDate datetime = NULL -- Use either @CleanupBeforeDate or @HoursToKeep BUT not both
