...
-- ### [Hash]: 3311d03 [SHA256-0A35E6AD163637FF24DF379A8AC2327C2CCAA0070125C8469F20EF096BFC9A30]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
-- ### [Hash]: f12ceeb [SHA256-5101993CB675CC7DEE2E5BEF3C50BAC46D5A5654A672DAA88CA3EB37F3721DF0]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
-- ### [Hash]: db87142 [SHA256-3B7DD6AE9B36F68F91F626C826A0FFC12FAA381120B2A497E801D489D09B20EF]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
-- ### [Hash]: f12ceeb [SHA256-3ECC2B10D8C05FD57074B303F5A6C9615C5BC6E72CC78EAB9990DDE366F676BA]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
--        EXEC sp_executesql @stmt = @stmtGetProcInfo, @params = @paramsGetProcInfo, @procid = @@PROCID, @info = N'Version', @output = @versionDatetime OUTPUT;
       ...
       INSERT INTO @messages([Message], Severity, [State])
        ...
        IF NOT EXISTS(SELECT 1 FROM sys.tables WHERE [name] = N'QueueItems' AND SCHEMA_NAME(schema_id) = N'dbo' AND is_external = 1)
        BEGIN
            SET @isExternal = 0;
    ...
    BEGIN TRY 
        ...
        BEGIN
            ...
            BEGIN TRY
                ...
                IF @isExternal = 0
                BEGIN
--                    EXEC sp_executesql @stmt = N'SELECT @maxId = MAX(Id) FROM [Maintenance].[Synonym_Source_QueueItems] WITH(INDEX([IX_TenantId_IsGlobal_ExecutionTime])) WHERE ExecutionTime <= @targetTimestamp;', @params = N'@maxId bigint OUTPUT, @targetTimestamp datetime', @maxId = @maxId OUTPUT, @targetTimestamp = @targetTimestamp;
                    SELECT @maxId = MAX(Id) FROM [Maintenance].[Synonym_Source_QueueItems];
                END
                ELSE
                BEGIN
                    SELECT @maxId = MAX(Id) FROM [Maintenance].[Synonym_Source_QueueItems];
...
--                      WHERE ( (src.ExecutionTime >= flt.PreviousTimestamp AND src.ExecutionTime < flt.TargetTimestamp) OR (flt.LastId IS NOT NULL AND src.ExecutionTime < flt.PreviousTimestamp AND src.Id > flt.LastId ) ) AND src.Id >= flt.CurrentId AND src.Id <= @maxId AND src.Id >= @currentId
                        ...
                        WHERE NOT EXISTS(SELECT 1 FROM [Maintenance].[Delete_Queues] WHERE Id = ids.tempId) AND tempNoDelay = 0;             --|
                                                                                                                                             --|
                        IF @isExternal = 0 DELETE src FROM #tempListIds ids INNER JOIN [Maintenance].[Synonym_Source_QueueItemComments] src ON src.QueueItemId = ids.tempId WHERE @ignoreDelay = 1 OR ids.tempNoDelay = 1;
                        IF @isExternal = 0 DELETE src FROM #tempListIds ids INNER JOIN [Maintenance].[Synonym_Source_QueueItemEvents] src ON src.QueueItemId = ids.tempId WHERE @ignoreDelay = 1 OR ids.tempNoDelay = 1;
                        IF @isExternal = 0 DELETE src FROM #tempListIds ids INNER JOIN [Maintenance].[Synonym_Source_QueueItems] src ON src.Id = ids.tempId WHERE @ignoreDelay = 1 OR ids.tempNoDelay = 1;
                                                                                                                                             --|
                        -- Update current Id(s)                                                                                              --|
...
-- ### [Hash]: 3311d03 [SHA256-37EB7AEE757E731B1281371CB03A120431FFA59899C920B7F7E707D805D29BD8]
-- ### [Docs]: https://???.???
-- !!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!!
-- !!! ~~~~~~~~~ NOT OFFICIALLY SUPPORTED BY UIPATH 
-- !!! ~~~~~~~~~ SQL Server >= 2016 SP1
...
/*            -- Check Dry Run
    ...
    BEGIN TRY 
        ...
        BEGIN TRY
            ...
            FROM [Maintenance].[Synonym_Source_ASyncStatus_Queues] sts
